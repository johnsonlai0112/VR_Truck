...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    public class XRKnob : XRBaseInteractable
    {
        ...
        [SerializeField]
        [Tooltip("Duration in seconds to reset the knob value back to the original value")]
        private float resetDuration = 2.0f; // Time in seconds to reset the knob value
        ...
        private Coroutine resetCoroutine; // Coroutine reference
        ...
        void EndGrab(SelectExitEventArgs args)
        {
            ...
            // Start the coroutine to reset the knob value
            if (resetCoroutine != null)
            {
                StopCoroutine(resetCoroutine);
            }
            resetCoroutine = StartCoroutine(ResetKnobValue());
        }

        private IEnumerator ResetKnobValue()
        {
            ...
            while (elapsedTime < resetDuration)
            {
                ...
                m_Value = Mathf.Lerp(startValue, endValue, t);
                SetKnobRotation(ValueToRotation());
                m_OnValueChange.Invoke(m_Value);
                yield return null;
            }

            // Ensure the final value is set to 0.5
            m_Value = endValue;
            SetKnobRotation(ValueToRotation());
            m_OnValueChange.Invoke(m_Value);
            resetCoroutine = null;
